...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                pi_latent = self.vf_latent = cnn_extractor(self.processed_obs, **kwargs)
            else:
                pi_latent, self.vf_latent = mlp_extractor(tf.layers.flatten(self.processed_obs), net_arch, act_fun)

            self._value_fn = linear(self.vf_latent, 'vf', 1)

            self._proba_distribution, self._policy, self.q_value = \
                self.pdtype.proba_distribution_from_latent(pi_latent, self.vf_latent, init_scale=0.01)

        self._setup_init()

    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, vf_latent, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.vf_latent, self.neglogp],
                                                   {self.obs_ph: obs})
        else:
            action, value, vf_latent, neglogp = self.sess.run([self.action, self.value_flat, self.vf_latent, self.neglogp],
                                                   {self.obs_ph: obs})
        return action, value, self.initial_state, vf_latent, neglogp
...
class SmallMlpPolicy(FeedForwardPolicy):
    """
    Policy object that implements actor critic, using a MLP (2 layers of 64)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(SmallMlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                             net_arch=[dict(vf=[16], pi=[16])], feature_extraction="mlp", **_kwargs)
